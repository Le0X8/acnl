...
switch (mode) {
  case 'exhibition(wa)':
    ...
    const exhibitionWa = parseExhibitionDatWa(
      exhibitionDatWa as unknown as Buffer,
    );
  ...
  case 'mydsgn':
    ...
    if (!existsSync(process.argv[2] + '/mydsgn'))
      mkdirSync(process.argv[2] + '/mydsgn');
    if (!existsSync(process.argv[2] + '/mydsgn/a'))
      mkdirSync(process.argv[2] + '/mydsgn/a');
    if (!existsSync(process.argv[2] + '/mydsgn/b'))
      mkdirSync(process.argv[2] + '/mydsgn/b');
    if (!existsSync(process.argv[2] + '/mydsgn/c'))
      mkdirSync(process.argv[2] + '/mydsgn/c');
