...
export function parseVillagerHome(data: Buffer) {
  return {
    ...
    unknown0x00062: Array.from({ length: 21 }, (_, i) =>
      parseItem(
        data.subarray(0x00062 + i * 4, 0x00062 + (i + 1) * 4) as Buffer,
      ),
    ),
    ...
    unknown0x000de: Array.from({ length: 4 }, (_, i) =>
      parseDate(
        data.subarray(0x000de + i * 4, 0x000de + (i + 1) * 4) as Buffer,
      ),
    ),
