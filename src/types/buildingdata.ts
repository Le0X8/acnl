...
export function parseBuildingData(buffer: Buffer) {
  return {
    ...
    buildings: Array.from({ length: 56 }, (_, i) => parseBuilding(buffer.subarray(0x00008 + i * 4, 0x000e8 + i * 4) as Buffer)),
    eventBuildings: Array.from({ length: 2 }, (_, i) => parseBuilding(buffer.subarray(0x000e8 + i * 4, 0x000f0 + i * 4) as Buffer)),
    designStandPatterns: Array.from({ length: 8 }, (_, i) => parseDesignStandPattern(buffer.subarray(0x000f0 + i * 0x787, 0x044b0 + i * 0x787) as Buffer)),
