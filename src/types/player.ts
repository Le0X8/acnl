...
export function parsePlayer(buffer: Buffer) {
  return {
    ...
    patterns: Array.from({ length: 10 }, (_, i) => parsePattern(buffer.subarray(0x0002c + i * 0x870, 0x0002c + (i + 1) * 0x870) as Buffer)),
    ...
    mannequins: Array.from({ length: 4 }, (_, i) => parseMannequin(buffer.subarray(0x05544 + i * 0x18, 0x05544 + (i + 1) * 0x18) as Buffer)),
    ...
    playerBadges: parsePlayerBadges(buffer.subarray(0x055dc, 0x056c4) as Buffer),
    hhaHouseInfo: parseHhaHouseInfo(buffer.subarray(0x056c4, 0x056f0) as Buffer),
    dreamAddress: parseDreamAddress(buffer.subarray(0x056f0, 0x056fc) as Buffer),
    ...
    bellsFromReese: parseEncryptedValue(buffer.subarray(0x06ba4, 0x06bac) as Buffer),
    ...
    inventory: Array.from({ length: 16 }, (_, i) => parseItem(buffer.subarray(0x06bd0 + i * 0x04, 0x06bd0 + (i + 1) * 0x04) as Buffer)),
    ...
    islandBox: Array.from({ length: 40 }, (_, i) => parseItem(buffer.subarray(0x06f10 + i * 0x04, 0x06f10 + (i + 1) * 0x04) as Buffer)),
    islandInventory: Array.from({ length: 16 }, (_, i) => parseItem(buffer.subarray(0x06fb0 + i * 0x04, 0x06fb0 + (i + 1) * 0x04) as Buffer)),
    ...
    playerLetters: Array.from({ length: 10 }, (_, i) => parseLetter(buffer.subarray(0x07008 + i * 0x280, 0x07008 + (i + 1) * 0x280) as Buffer)),
    ...
    meowCoupons: parseEncryptedValue(buffer.subarray(0x08d1c, 0x08d24) as Buffer),
    unknown0x08d24: parseEncryptedValue(buffer.subarray(0x08d24, 0x08d2c) as Buffer),
    unknown0x08d2c: parseEncryptedValue(buffer.subarray(0x08d2c, 0x08d34) as Buffer),
    unknown0x08d34: parseEncryptedValue(buffer.subarray(0x08d34, 0x08d3c) as Buffer),
    unknown0x08d3c: parseEncryptedValue(buffer.subarray(0x08d3c, 0x08d44) as Buffer),
    unknown0x08d44: parseEncryptedValue(buffer.subarray(0x08d44, 0x08d4c) as Buffer),
    unknown0x08d4c: Array.from(buffer.subarray(0x08d4c, 0x08f08)),
    santaBag: Array.from({ length: 10 }, (_, i) => parseItem(buffer.subarray(0x08fa8 + i * 0x4, 0x08fa8 + (i + 1) * 0x4) as Buffer)),
    unknown0x08fd0: Array.from(buffer.subarray(0x08fd0, 0x08fd0 + 0x320)),
    dressers: Array.from({ length: 180 }, (_, i) => parseItem(buffer.subarray(0x092f0 + i * 0x4, 0x092f0 + (i + 1) * 0x4) as Buffer)),
    birthdayWish: buffer.toString('utf16le', 0x095c0, 0x09604),
    unknown0x09604: Array.from({ length: 5 }, (_, i) => parseLetter(buffer.subarray(0x09604 + i * 0x280, 0x09604 + (i + 1) * 0x280) as Buffer)),
