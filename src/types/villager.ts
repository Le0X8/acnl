...
export function parseVillager(data: Buffer) {
  return {
    ...
    unknown0x008c6: Array.from({ length: 10 }, (_, i) =>
      parseItem(
        data.subarray(0x008c6 + i * 4, 0x008c6 + (i + 1) * 4) as Buffer,
      ),
    ),
    homes: Array.from({ length: 16 }, (_, i) =>
      parseVillagerHome(
        data.subarray(
          0x008ee + i * 0x00f2,
          0x008ee + (i + 1) * 0x00f2,
    ...
    unknown0x02490: Array.from({ length: 5 }, (_, i) =>
      data.readUInt16LE(0x02490 + i * 2),
    ),
    ...
    furniture: Array.from({ length: 16 }, (_, i) =>
      parseItem(
        data.subarray(0x024ae + i * 4, 0x024ae + (i + 1) * 4) as Buffer,
      ),
    ),
    unknown0x024ee: parseDate(data.subarray(0x024ee, 0x024f2) as Buffer),
    catchphrase: data.toString('utf16le', 0x024f2, 0x024f2 + 0x16),
    unknown0x02508: Array.from({ length: 2 }, (_, i) =>
      data.readUInt8(0x02508 + i),
    ),
    unknown0x0250a: parseDate(data.subarray(0x0250a, 0x0250e) as Buffer),
    unknown0x0250e: Array.from({ length: 2 }, (_, i) =>
      data.readUInt8(0x0250e + i),
    ),
    unknown0x02510: Array.from({ length: 2 }, (_, i) =>
      data.readUInt32LE(0x02510 + i * 4),
    ),
  };
}
